...
```text
gtex62-core/providers/time/fetch_time.sh (profile: local)
  → ~/.cache/gtex62-core/shared/time/local/current.json   (rows with zone, name, time, date)
    → lua/suite/tme.lua                                   (reads JSON via parse_time_rows)
      → lua/ui/frame.lua                                  (draws TME panel)
```

**Important:** The clock row names (e.g. "UNIVERSAL TIME") come from `current.json`, generated by `fetch_time.sh`. The `DISPLAY_ZONES` table in `tme.lua` is only a fallback used when the JSON file is absent. Changing a name in `tme.lua` alone will have no visible effect if `current.json` exists.

To change a timezone display name, update it in **both**:

1. `~/.config/gtex62-core/profiles/time/local.toml` — `[zones]` section (authoritative)
2. `gtex62-core/providers/time/fetch_time.sh` — `DEFAULT_ZONES` list (fallback)
3. `lua/suite/tme.lua` — `DISPLAY_ZONES` (Lua fallback when JSON absent)

Then force-refresh the cache:

```bash
bash ~/.config/conky/gtex62-core/providers/time/fetch_time.sh local
```

### Symptoms

- Clock timezone name wrong in display
- Timezone row missing
- Time values not updating
...
| Data module | `lua/suite/tme.lua` |

### First Mechanical Checks

- What name is actually in the cache? (`grep name ~/.cache/gtex62-core/shared/time/local/current.json`)
- Is the profile TOML `[zones]` section overriding the default? (`cat ~/.config/gtex62-core/profiles/time/local.toml`)
- Does the cache update after running the provider manually?
